...
func (cmd commandCwd) Execute(conn *Conn, param string) {
	...
	if err == nil {
		...
		conn.writeMessage(450, "Action not taken "+err.Error())
	}
}
...
func (cmd commandDele) Execute(conn *Conn, param string) {
	...
	if err == nil {
		...
		conn.writeMessage(550, "Action not taken "+err.Error())
	}
}
...
func (cmd commandNlst) Execute(conn *Conn, param string) {
	...
	info, err := conn.driver.Stat(path)
	if err != nil {
		conn.writeMessage(502, err.Error())
		return
	}
	if !info.IsDir() {
...
func (cmd commandPass) Execute(conn *Conn, param string) {
	ok, err := conn.server.Auth.CheckPasswd(conn.driver, conn.reqUser, param)
	if err != nil {
...
func (cmd commandRnto) Execute(conn *Conn, param string) {
	...
	if err == nil {
		...
		conn.writeMessage(502, "Action not taken "+err.Error())
	}
}
